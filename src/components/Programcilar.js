...
export default function Programcilar() {
  ...
  const [programcılar, setProgramcılar] = useState(enIyilerListesi);
  const [birinciID, setbirinciID] = useState(null);
  ...
  const oneCikaninIsmi = () => {
    ...
    const birinci = programcılar.find((dev) => dev.id === birinciID);
    return birinci ? birinci.isim : '';
  };

  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color:     birinciID ? "gold":'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          " */
          programcılar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setbirinciID(dev.id)}}>Kutla</button>
            </div>
          )
        }
      </div>
